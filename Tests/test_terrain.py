...
def test_get_terrain_data():
    out_path = Path(repo / r"Tests/Test_Data/test_terrain/out")
    ghs_path = Path(
        repo,
        "Tests",
        "Test_Data",
    ...
    for r in rcit:
        ...
        with pytest.raises(rasterio.errors.RasterioIOError):
            fout_path = "fred"
            get_terrain_data(bounds, zoom, fout_path, city_name, delfile=True)
        with pytest.raises(rasterio.errors.RasterioIOError):
            fout_path = Path("fred")
            get_terrain_data(bounds, zoom, fout_path, city_name, delfile=True)
...
def test_decode_terrain_png():
    base_path = Path(repo / r"Tests/Test_Data/test_terrain")
    ghs_path = Path(
        repo,
        "Tests",
        "Test_Data",
    ...
    tile_path = Path(base_path, "decode_terrain_png", "Test_Random_Raster.tif")
    with pytest.raises(IndexError):
        decode_terrain_png(str(tile_path))
    # test absent file
    tile_path = Path(base_path, "out", "fred.gpkg")
