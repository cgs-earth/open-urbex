...
os.chdir(repo)
test_data_path = repo / "Scripts" / "Tests" / "Test_Data" / "test_rasters"
...
def test_run_kde():
    ...
    # bad bounding box
    a, yct, xct = run_kde(points, 1, 2, 2, 1)
    assert (
        isinstance(a, np.ndarray)
        and isinstance(yct, np.ndarray)
        and isinstance(xct, np.ndarray)
...
def test_node_dist_grid():
    ...
    assert abs(gx.max() - max(xpts)) < 0.15
    assert abs(gx.min() - min(xpts)) < 0.15
    assert abs(gy.max() - max(ypts)) < 0.15
    assert abs(gy.min() - min(ypts)) < 0.15
