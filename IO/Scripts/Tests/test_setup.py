...
base_path = repo / "Scripts" / "Tests" / "Test_Data" / "test_setup"
...
def test_local_import():
    ...
    with pytest.raises(pyogrio._io.DataSourceError):
...
def test_wgs84_to_utm():
    ...
    UTMZone2, wkid2 = wgs84_to_utm(ggg2)
    assert UTMZone != UTMZone2
    assert wkid != wkid2

    # test dif crs same loc
    ggg3 = ggg.to_crs(3857)
    UTMZone3, wkid3 = wgs84_to_utm(ggg3)
...
def test_folder_set_up():
    ...
    outfp, fpdict = folder_set_up(temp_dir)
    for fp in fpdict:
...
def test_test_set_up():
    temp_dir = Path(tempfile.mkdtemp())
    outfp, fpdict = test_set_up(temp_dir)
    for fp in fpdict:
        assert Path(fp).exists()
        assert Path(outfp).exists()
