...
def node2cc(rds, cc, wkid):  # TODO: Write Test!
    """
    ...
    cc["edge_index"] = mp.get_nearest_street(cc, edges)
    city_center = mp.get_nearest_node(cc, nodes, edges, cc["edge_index"])  # noqa
    citcen = [
        tuple(x)
        for x in nodes.query("nodeID.isin(@city_center)")[["x", "y"]].values.tolist()
    ]
    # solve graph
    dj = nx.single_source_dijkstra_path_length(
        G, source=citcen[0], weight="mm_len"
    )  # distance is in meters
    # attach distances to nodes dataframe
    nodes["tup_node"] = [tuple(x) for x in nodes[["x", "y"]].values.tolist()]
    dj_df = pd.DataFrame.from_dict(dj, orient="index").reset_index()
    dj_df.columns = ["tup_node", "cc_dist"]
    nodes = pd.merge(nodes, dj_df, how="left", on="tup_node")
